<div>
    ...
    @foreach($steps as $key => $value)
        <div wire:key="{{$key}}" class="flex">
            <input type="text" name="steps[]" value="{{ $value }}" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md mb-3 mr-2" placeholder="{{ __('step.description') }}" required>
